...
HTML added via the `data-content` attribute on individual options is sanitized using the built-in zero-dependency sanitizer.
...
If you prefer to use a dedicated sanitizer library such as DOMPurify, load or install it in your application and wire it through `sanitizeFn`:
...
For performance reasons, bootstrap-select passes an array of DOM nodes to `sanitizeFn`, not an HTML string. If you use a third-party sanitizer, sanitize each node in place as shown above.
